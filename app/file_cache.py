...
class FileCache(sky_object.SkyObject):
    def __init__(self, name):
        ...
        self.cache_root = f"./orbit_data/{self.name}"
    ...
    def get_local_filename(self, utc_date):
        return f"{utc_date.year}/{utc_date.month}/{utc_date.day}.dat"
            ...
            if os.path.exists(self.ban_file):
                ...
                if datetime.datetime.now() < ban_until_with_buffer:
                    if datetime.datetime.now() < ban_until:
                        ...
                        logging.info(
                            f"Ban expired at {ban_until}, waiting 5 min buffer until {ban_until_with_buffer}"
                        )
                    return True
    ...
    def set_ban(self, hours=3):
        ...
        try:
            ...
            logging.exception(f"Error setting ban: {e}")
        ...
        try:
            ...
            for days_back in range(7):
                ...
                if os.path.isfile(local_path):
                    logging.info(
                        f"Using cached file from {days_back} days ago: {local_path}"
                    )
                    return local_path, fname
        except Exception as e:
            logging.exception(f"Error finding cached file: {e}")
        return None, None
    ...
    def download_file(self, url, local_file):
        ...
        try:
            ...
            logging.info(f"starting download ({url} -> {local_file}")
            ...
            if err.code == 403:
                logging.exception("403 Forbidden - Setting ban for 3 hours")
    ...
    def get_object(self, date):
        ...
        try:
            ...
            logging.exception("Download failed. Looking for cached data...")
            ...
            if os.path.isfile(local_path):
                try:
                    ...
                    logging.exception(f"Failed to load existing file {local_path}: {e}")
            ...
            if cached_path and cached_fname:
                try:
                    if cached_fname not in self.cache:
                        self.cache[cached_fname] = self.create_object_from_file(
                            cached_path
                        )
                    ...
                    logging.exception(f"Failed to load cached file {cached_path}: {e}")

            # If all else fails, raise the original error
            logging.exception("No cached data available, re-raising original error")
            raise error
