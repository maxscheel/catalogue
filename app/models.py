...
class BulkAzElRequest(BaseModel):
    ...
    lat: float = Field(
        ..., description="Observer latitude in degrees", example=-45.85, ge=-90, le=90
    )
    lon: float = Field(
        ...,
    ...
    elevation: float | None = Field(
        0.0,
        description="Minimum elevation cutoff in degrees (satellites below this are filtered out)",
        example=10.0,
        ge=0,
        le=90,
    )
    ...
    dates: list[str] = Field(
        ...,
        description="List of ISO format datetime strings",
        example=["2024-01-15T12:00:00Z", "2024-01-15T12:01:00Z"],
    )
...
class SatelliteInfo(BaseModel):
    ...
    name: str = Field(
        ..., description="Satellite name or identifier", example="GPS BIIR-2"
    )
    az: float = Field(
        ...,
        description="Azimuth angle in degrees (0=North, 90=East)",
        example=125.5,
        ge=0,
        lt=360,
    )
    el: float = Field(
        ...,
        description="Elevation angle in degrees above horizon",
        example=45.2,
        ge=0,
        le=90,
    )
    r: float = Field(
        ..., description="Range/distance to satellite in meters", example=20234567.8
    )
    jy: float | None = Field(
        None, description="Flux density in Jansky for radio astronomy", example=1e-4
    )
...
class PositionInfo(BaseModel):
    ...
    ecef: list[float] = Field(
        ...,
        description="ECEF position coordinates [x, y, z] in meters",
        example=[12345678.9, -23456789.1, 34567890.2],
    )
    ecef_dot: list[float] = Field(
        ...,
        description="ECEF velocity vector [vx, vy, vz] in m/s",
        example=[1234.5, -2345.6, 3456.7],
    )
    jy: float = Field(
        ..., description="Flux density in Jansky for radio astronomy", example=1e-4
    )
...
class BulkAzElResponse(BaseModel):
    ...
    dates: list[str] = Field(
        ..., description="Input timestamps", example=["2024-01-15T12:00:00Z"]
    )
    az_el: list[list[SatelliteInfo]] = Field(
        ..., description="Catalog data for each timestamp - array of satellite arrays"
    )
...
class ErrorResponse(BaseModel):
    ...
    detail: str = Field(
        ..., description="Error description", example="No catalog data found"
    )
